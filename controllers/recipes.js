...
const getAll_recipes = async (req, res, next) => {
  try {
...
const getSingle_recipes = async (req, res, next) => {
    try {
...
const getSingle_recipes_by_recipe_title = async (req, res, next) => {
  try {
    const recipeTitle = req.params.recipe_title;
    const result = await mongodb.getCluster().db().collection('Recipes').find({ recipe_title: recipeTitle });
    result.toArray().then((lists) => {
      res.setHeader('Content-Type', 'application/json');
      res.statusCode = 200;
      res.json(lists[0]);
...
const post_recipes = async (req, res, next) => {
...
const update_recipes = async (req, res, next) => {
  const recipeId = new ObjectId(req.params.id);
  const data = {
    "recipe_title": req.body.recipe_title,
    "description": req.body.description,
  ...
  const response = await mongodb.getCluster().db().collection('Recipes')
...
const delete_recipes = async (req, res, next) => {
  const recipeId = new ObjectId(req.params.id);
  const response = await mongodb.getCluster().db().collection('Recipes').deleteOne({ _id: recipeId });
